...
import { User, Bell, Edit, Home, Briefcase, MessageSquare, ChevronRight } from 'lucide-react'
...
export default function JobSeekerHomepage({ user }: JobSeekerHomepageProps) {
  const [userProfile, setUserProfile] = useState<UserProfile | null>(null)
  const [applicationStats, setApplicationStats] = useState<ApplicationStats>({ applications: 0 })
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [profileRes, statsRes, notificationsRes, internshipsRes] = await Promise.all([
      ...
      if (notificationsRes.ok) {
        ...
        setNotifications([
          { id: '1', message: 'New job match found for your skills', createdAt: new Date().toISOString(), read: false },
          { id: '2', message: 'Application status updated for Frontend Developer role', createdAt: new Date().toISOString(), read: false },
          { id: '3', message: 'Profile viewed by 3 employers this week', createdAt: new Date().toISOString(), read: true }
        ])
      }
  ...
  const loadMoreInternships = async () => {
    try {
      setLoadingMore(true)
      const response = await fetch(`/api/dashboard/recommendations?limit=4&offset=${internships.length}`)
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between h-16">
              <div className="flex items-center">
                <span className="text-2xl font-bold text-blue-600">WorkQit</span>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            ...
            <div className="flex items-center space-x-8">
              ...
              <Link href="/jobs" className="flex items-center text-gray-700 hover:text-blue-600 transition-colors">
                <Briefcase className="w-4 h-4 mr-1" />
                Opportunities
              </Link>
              <Link href="/profile" className="flex items-center text-gray-700 hover:text-blue-600 transition-colors">
                <User className="w-4 h-4 mr-1" />
                My Profile
              </Link>
              <Link href="/messages" className="flex items-center text-gray-700 hover:text-blue-600 transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="text-center">
                ...
                {/* Career Interests */}
                <div className="mb-6">
                  <h4 className="text-sm font-medium text-gray-700 mb-3">Career Interests</h4>
                  <div className="flex flex-wrap gap-2">
                    {userProfile?.profile?.skills?.length ? (
                      userProfile.profile.skills.slice(0, 4).map((skill, index) => (
                        <span 
                          key={index}
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-sm p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-6">Recommended Internships</h2>
                  ...
                  internships.map((internship) => (
                    <div key={internship._id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                      ...
                      <p className="text-sm text-gray-700 mb-4 line-clamp-2">
                        {internship.description || 'Exciting opportunity to grow your career and gain valuable experience.'}
                      </p>
